...
from decision_transformer.models.gru import GRUModel
...
def experiment(
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        s, a, r, d, rtg, timesteps, mask, tlens = [], [], [], [], [], [], [], []
        for i in range(batch_size):
            ...
            if model_type == "gru":
                s[-1] = np.concatenate(
                    [s[-1], np.zeros((1, max_len - tlen, state_dim))], axis=1
                )
                s[-1] = (s[-1] - state_mean) / state_std
                a[-1] = np.concatenate(
                    [a[-1], np.ones((1, max_len - tlen, act_dim)) * -10.0], axis=1
                )
                r[-1] = np.concatenate(
                    [r[-1], np.zeros((1, max_len - tlen, 1))], axis=1
                )
                d[-1] = np.concatenate(
                    [d[-1], np.ones((1, max_len - tlen)) * 2], axis=1
                )
                rtg[-1] = (
                    np.concatenate([rtg[-1], np.zeros((1, max_len - tlen, 1))], axis=1)
                    / scale
                )
                timesteps[-1] = np.concatenate(
                    [timesteps[-1], np.zeros((1, max_len - tlen))], axis=1
                )
                mask.append(
                    np.concatenate(
                        [np.ones((1, tlen)), np.zeros((1, max_len - tlen))], axis=1
                    )
                )
            else:
                s[-1] = np.concatenate(
        ...
        tlens = torch.tensor(tlen, dtype=torch.long, device="cpu")

        return s, a, r, d, rtg, timesteps, mask, tlens
